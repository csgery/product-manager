  ...
  Mutation: {
    ...
    updateProduct: async (
      ...
      try {
        ...
        throw productNotChanged_Error();
      } catch (err) {
        throw err;
      }
    },
    deleteProduct: async (parent, { id }, { user: userCtx }) => {
      try {
        ...
        if (!product.valid) {
          throw productAlreadySoftDeleted_Error();
        }
    ...
    restoreDeletedProduct: async (parent, { id }, { user: userCtx }) => {
      try {
        const product = await Product.findById(id);
        if (!product) {
          throw productNotFound_Error();
        ...
        if (product.valid) {
          throw productAlreadyValid_Error();
        }
    ...
    removeProduct: async (parent, { id }, { user: userCtx }) => {
      try {
        const product = await Product.findById(id);
        if (!product) {
          throw productNotFound_Error();
        ...
        if (product.valid) {
          throw productSoftDeleteFirst_Error();
        }
